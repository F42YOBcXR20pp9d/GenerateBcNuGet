Write-Host "Determine Artifacts (Releases Only)"
...
if ($artifactVersion -eq '' -or $artifactVersion.EndsWith('-')) {
    ...
    if ($newPackage) {
        ...
        $artifactVersions = @(GetArtifactVersionsNeeded -apps $apps -allArtifactVersions $allArtifactVersions -runtimeDependencyPackageIds $runtimeDependencyPackageIds -nuGetServerUrl $nuGetServerUrl -nuGetToken $token)
    }
}
else {
